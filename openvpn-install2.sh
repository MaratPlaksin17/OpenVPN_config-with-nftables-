...
nft add chain ip my_table my_chain '{type nat hook postrouting priority 1;}'
nft add rule ip my_table my_chain ip saddr 10.8.0.0/24 oif $NIC masquerade
nft add chain ip my_table my_chain1 '{type filter hook input priority 1; policy accept;}'
nft add rule ip my_table my_chain1 iif tun0 accept
nft add chain ip my_table my_chain2 '{type filter hook forward priority 1; policy accept;}'
nft add rule ip my_table my_chain2 oif tun0 iif $NIC accept
nft add chain ip my_table my_chain3 '{type filter hook forward priority 1; policy accept;}'
nft add rule ip my_table my_chain3 oif $NIC iif tun0 accept
nft add chain ip my_table my_chain4 '{type filter hook input priority 1; policy accept;}'
nft add rule ip my_table my_chain4 iif $NIC $PROTOCOL dport $PORT accept">/etc/nftables/add-openvpn-rules.sh
...
#ptables -D FORWARD -i $NIC -o tun0 -j ACCEPT
#iptables -D FORWARD -i tun0 -o $NIC -j ACCEPT
#iptables -D INPUT -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >/etc/iptables/rm-openvpn-rules.sh
echo "#!/bin/sh
nft delete rule ip my_table my_chain
nft delete rule ip my_table my_chain1
nft delete rule ip my_table my_chain2
nft delete rule ip my_table my_chain3
nft delete rule ip my_table my_chain4">/etc/nftables/rm-openvpn-rules.sh
...
#	fi
	chmod +x /etc/nftables/add-openvpn-rules.sh
	chmod +x /etc/nftables/rm-openvpn-rules.sh
	# Handle the rules via a systemd script
	echo "[Unit]
Description=nftables rules for OpenVPN
Before=network-online.target
Wants=network-online.target
[Service]
Type=oneshot
ExecStart=/etc/nftables/add-openvpn-rules.sh
ExecStop=/etc/nftables/rm-openvpn-rules.sh
RemainAfterExit=yes
[Install]
WantedBy=multi-user.target" >/etc/systemd/system/nftables-openvpn.service
	# Enable service and apply rules
	systemctl daemon-reload
	systemctl enable nftables-openvpn
	systemctl start nftables-openvpn
